...
6. **Write design doc** — save to `docs/superpowers/specs/YYYY-MM-DD-<topic>-design/` folder and commit
...
digraph brainstorming {
    ...
    "Write design doc\n(folder + overview.md + sections)" [shape=box];
    ...
    "User approves design?" -> "Write design doc\n(folder + overview.md + sections)" [label="yes"];
    "Write design doc\n(folder + overview.md + sections)" -> "Simple spec?\n(single concept, <500 words)";
    ...
    "User reviews spec?" -> "Write design doc\n(folder + overview.md + sections)" [label="changes requested"];
...
- Write the validated design (spec) to `docs/superpowers/specs/YYYY-MM-DD-<topic>-design/`:
  - `overview.md` — problem statement, goals, approach chosen, key constraints, out-of-scope
  - One file per major design section (e.g. `architecture.md`, `data-model.md`, `api.md`, `testing.md`).
    Scale the number of section files to complexity — a simple spec may only need one `design.md` alongside `overview.md`.
...
> "Spec written and committed to `<folder-path>/`. Please review it and let me know if you want to make any changes before we start writing out the implementation plan."
